 ...
 return (
  <Container>
      ...
      <Content>
        ...
        <Paragraph style={{backgroundColor:'#4169E1'}}>WORLD</Paragraph>
      </Content>
      <PostContainer>
                    ...
                    if(i===0){
                     return ( <Children key={i}  css={css`width:550px; @media ${device.mobile}{width:100%;} `}>                                            
                           <Image src={typeof attachment==='object' ? attachment['source_url'] :'https://www.newspitara.com/wp-content/plugins/td-composer/legacy/Newspaper/assets/images/no-thumb/td_696x0.png'} css={css`width:550px; @media ${device.mobile}{width:100%;} `} />
                            <div  css={css`position:absolute;margin-left:10px;top:210px;color:white; @media ${device.mobile}{width:100%;}`}>
                       ...
                       return ( <Children key={i} css={css`width:550px; @media ${device.mobile}{width:100%;} `}>                                                
                        <Image src={typeof attachment==='object' ? attachment['source_url'] :'https://www.newspitara.com/wp-content/plugins/td-composer/legacy/Newspaper/assets/images/no-thumb/td_696x0.png'} css={css`width:550px; @media ${device.mobile}{width:100%;} `}/>
                            <div css={css`position:absolute;margin-left:10px;top:220px;color:white; @media ${device.mobile}{width:100%;}`}>
                                <p style={{fontSize:'0.7rem'}}>{category.name}</p>
                                <Link href={post.link+post.id}>
                                    <h1  css={css`color:white;margin-top:5px;font-size:18px;width:30rem;text-transform: capitalize; @media ${device.mobile}{width:95%;}`}>{link}</h1>
                                </Link>
                                <p style={{fontSize:'12px',marginTop:'5px'}}>{author.name}-{new Date(post.date).toLocaleDateString('en-Us',{month:'long',day:'2-digit',year:'numeric'})}</p>
                            </div>                         
                        </Children>)
                       ...
                       return ( <Children key={i} css={css`width:370px; &:hover{text-decoration:underline;} @media ${device.mobile}{width:100%;margin-left:10px;} `} >                                                
                           <Image src={typeof attachment==='object' ? attachment['source_url'] :'https://www.newspitara.com/wp-content/plugins/td-composer/legacy/Newspaper/assets/images/no-thumb/td_696x0.png'} css={css`width:350px;transform:scale(1); @media ${device.mobile}{width:40%;height:15vh;float:left;}`}/>
                            <div css={css`position:relative;marginLeft:10px;color:white;@media ${device.mobile}{float:left; width:57%;height:auto; margin-left:10px; }`}>
                                <p style={{fontSize:'0.7rem',color:'#4169E1'}}>{category.name}</p>
                                <Link href={post.link+post.id}>
                                    <h1 css={css`color:black;margin-top:5px;font-size:18px;width:20rem;text-transform: capitalize; &:hover{text-decoration:underline;} @media ${device.mobile}{font-size:15px;font-weight:normal;width:100%;} `}>{link}</h1>
                                </Link>
