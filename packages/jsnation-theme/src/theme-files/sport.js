    ...
    <InnerContainer>
        ...
        @media ${device.mobile}{
            ...
            {typeof sport.items==='object' && sport.items.map((item,i)=>{
                        ...
                        if(i===0){
                            return ( <Children key={i} className='' css={css`color:black;width:400px;grid-column-start:1;grid-column-end:3; @media ${device.mobile}{width:100%;padding:15px;}`}>
                                    ...
                                    <div css={css`   width:500px;color:black;font-family:Arial;`}>
                                        ...
                                        <Link href={post.link+post.id}>
                                          <h1 className='link' css={css`font-size:15px;text-transform: capitalize;width:280px;color:black; &:hover{text-decoration:underline;} @media ${device.mobile}{font-size:18px; font-weight:normal;}`} >{link}</h1>
                                        </Link>
                                ...
                                return(<>
                                <Children key={i}  css={css`width:160px;  @media ${device.mobile}{width:100%;width:100%;padding:15px;}`}>
                                <img  src={typeof attachment==='object' ? attachment['source_url'] :'https://www.newspitara.com/wp-content/plugins/td-composer/legacy/Newspaper/assets/images/no-thumb/td_696x0.png'} css={css `width:140px; height:100px; @media ${device.mobile}{width:100%;height:25vh;}`}/>
                            ...
                            return(<>
                                <Children key={i}  css={css`width:430px;grid-column-start:3;grid-column-end:5 ;grid-row-start:1;grid-row-end:3; @media ${device.mobile}{height:100%; grid-column-start:1;grid-column-end:3;grid-row-start:3; grid-row-end:5;}`}>
                                    ...
                                    <div css={css`position:absolute; top:453px;color:white;margin-left:15px; @media ${device.mobile}{color:white;position:relative;top:-110px;left 300px;}`}>
                                        <p className='category' style={{color:'white'}}>{category.name}</p>
                                        <Link href={post.link+post.id}>
                                           <h1 className='link' css={css`color:white;font-weight:normal;margin-top:10px;font-size:16px;width:340px;&:hover{text-decoration:underline;} @media ${device.mobile}{font-size:1.2em; width:90%;}`}>{link}</h1>
                                        </Link>
                                        <p css={css`font-size:11px;`}>{author.name}-{new Date(post.date).toLocaleDateString('en-Us',{month:'long',day:'2-digit',year:'numeric'})}</p>
                                    </div>
                                </Children></>)
            ...
            <div css={css`display:grid;grid-template-columns:auto auto; grid-template-rows:auto auto; height:content-fit;gap:10px;marginTop:30px; @media ${device.mobile}{padding:15px; margin-top:10px;}`} >
                            ...
                            if(i===0){
                                return ( <Children key={i} className='' css={css`color:black;width:400px;grid-column-start:1;grid-column-end:3;`}>
                                        ...
                                        <div css={css`   width:500px;color:black;font-family:Arial;`}>
                                            ...
                                            <Link href={post.link+post.id}>
                                              <h1 className='link' css={css`font-size:15px;text-transform: capitalize;width:280px;color:black; &:hover{text-decoration:underline;}`}>{link}</h1>
                                            </Link>
                                            <p css={css`font-size:11px;width:160px;`}>{author.name}-{new Date(post.date).toLocaleDateString('en-Us',{month:'long',day:'2-digit',year:'numeric'})}</p>
                                        </div> 
                                    </Children>)
                                    ...
                                    return(<>
                                    <Children key={i}  css={css`width:160px; @media ${device.mobile}{height:100%;}`}>
                                    <img  src={typeof attachment==='object' ? attachment['source_url'] :'https://www.newspitara.com/wp-content/plugins/td-composer/legacy/Newspaper/assets/images/no-thumb/td_696x0.png'} css={css `width:140px; height:100px; @media ${device.mobile}{width:100%;height:25vh;}`}/>
