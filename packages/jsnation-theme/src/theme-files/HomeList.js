    ...
    <Container>
        {typeof data.items==='object' && data.items.map((item,i)=>{
                 ...
                 if(i===0){
                    ...
                    <img  src={typeof attachment==='object' ? attachment['source_url'] :'https://www.newspitara.com/wp-content/plugins/td-composer/legacy/Newspaper/assets/images/no-thumb/td_696x0.png'} css={css`height:200px;width:300px;@media ${device.mobile}{width:40%;height:80%;float:left;}`}/>
                <div css={css`@media ${device.mobile}{margin-left:10px;width:55%;float:left;}`}>
                       <p className='category' style={{color:'#4169E1'}}>{category.name}</p>
                       <Link href={post.link+post.id} >
                           <h1  css={css`font-size:18px;text-transform: capitalize;width:360px;font-weight:normal;color:black;margin-top:10px;fontSize:16px;width:270px;&:hover{text-decoration:underline;} @media ${device.mobile}{width:100%;font-size:14px;color:black;}`}>{link}</h1>
                       </Link>
